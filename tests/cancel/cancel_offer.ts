...
describe("CancelOffer", () => {
  ...
  beforeEach(async () => {
    ...
    const nativeOffer = await createNativeOffer(
      program,
      connection,
      wallet.payer,
      accounts,
    );
    const splOffer = await createSplOffer(
      program,
      connection,
      wallet.payer,
      accounts,
  ...
  it("should update balances canceling monochain sol sol offer", async () => {
    for (const offer of offers.filter(
      ...
      expect(
        (await connection.getBalance(accounts.srcSeller.publicKey)) -
          initialSellerBalance,
      ).to.be.closeTo(CREATE_OFFER_AMOUNTS.srcAmountLdNative, delta);
    }
  });
  ...
  it("should update balances canceling monochain spl spl offer", async () => {
    for (const offer of offers.filter(
      ...
      const initialSellerBalance = (
        await getBalance(connection, accounts.srcSellerAta)
      ).valueOf();

      const initialEscrowBalance = (
        await getBalance(connection, accounts.srcEscrowAta)
      ).valueOf();
